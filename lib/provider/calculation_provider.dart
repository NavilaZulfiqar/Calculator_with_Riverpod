...
class CalculationNotifier extends ChangeNotifier {
  ...
  final currentData = dataList.last;
  if (currentData.length > 1) {
    // Remove the last character from the current data
    dataList.last = currentData.substring(0, currentData.length - 1);
  } else if (dataList.length > 1) {
    // Remove the last element from dataList if the current data is a single character
    dataList.removeLast();
  }
  notifyListeners();
}
